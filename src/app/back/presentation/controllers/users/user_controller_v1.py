...
class UserController:
    ...
    def get_user_by_id(self, user_id: int, request: Request, x_trace_id: str = Depends(get_trace_id)) -> Result[UserResponse]:
        try:
    ...
    def create_user(self, create_request: CreateUserRequest, request: Request, x_trace_id: str = Depends(get_trace_id)):
        try:
            request.state.trace_logger = trace_logger.get_logger(x_trace_id)
            request.state.trace_logger.info("Guardando...")
            user_create_dto = UserMapper.request_to_dto(create_request)
            self.user_service.create_user(user_create_dto)
    ...
    def delete_user(self, user_id: int, request: Request, x_trace_id: str = Depends(get_trace_id)):
        try:
            request.state.trace_logger = trace_logger.get_logger(x_trace_id)
